    ...
    <div className="w-full bg-gray-50 min-h-[calc(100vh-72px)] py-8 md:py-10">
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-8 space-y-6 md:space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-6 border-b border-gray-200">
          ...
          <div className="inline-flex p-1.5 bg-gray-200/70 rounded-xl border border-gray-200 shadow-inner self-start md:self-end">
            <button
              type="button"
            ...
            >
              <Layers size={16} strokeWidth={activeTab === 'specs' ? 2.2 : 1.8} />
            ...
            >
              <FileSpreadsheet size={16} strokeWidth={activeTab === 'boq' ? 2.2 : 1.8} />
            ...
            >
              <Users2 size={16} strokeWidth={activeTab === 'contractors' ? 2.2 : 1.8} />
          ...
          <div className="space-y-6 md:space-y-8">
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-6 md:p-8 shadow-sm">
              <div className="flex items-center justify-between pb-4 mb-6 border-b border-gray-200">
                <div>
                  <span className="text-sm font-semibold uppercase tracking-wide text-gray-500 block mb-0.5">Foundations & Framing</span>
                ...
                <span className="hidden sm:inline-flex items-center px-3 py-1 rounded-full bg-green-50 border border-green-200 text-green-700 text-xs md:text-sm font-medium">
                  IS 456 Compliant
                </span>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {engineeringSpecs.map((spec, i) => (
                  <div
                    key={i}
                    className="group bg-white rounded-xl border border-gray-200 p-5 md:p-6 transition-all duration-200 hover:shadow-md hover:-translate-y-0.5 hover:border-gray-300 cursor-default"
                  >
                    <div className="text-xs font-medium uppercase tracking-wide text-gray-400">
                      {spec.label}
                    </div>
                    <div className="text-xl sm:text-2xl font-bold text-gray-900 mt-2 leading-snug">
                      {spec.value}
                    </div>
                  </div>
                ))}
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-6 md:p-8 shadow-sm">
              <div className="flex items-center justify-between pb-4 mb-6 border-b border-gray-200">
                <div>
                  <span className="text-sm font-semibold uppercase tracking-wide text-gray-500 block mb-0.5">Material Science & Geotech</span>
                  <h2 className="text-lg md:text-xl font-semibold text-gray-900">
        ...
        {activeTab === 'boq' && (
          <div className="bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm">
            <div className="p-6 md:p-8 border-b border-gray-200 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div>
                <span className="text-sm font-semibold uppercase tracking-wide text-gray-500 block mb-0.5">Financial Reconciliation</span>
                <h2 className="text-lg md:text-xl font-semibold text-gray-900">
              ...
              <div className="flex items-center gap-2 text-xs md:text-sm font-medium text-green-700 bg-green-50 px-3 py-1.5 rounded-full border border-green-200 self-start sm:self-auto">
              ...
              <table className="w-full text-left border-collapse min-w-[750px]">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-200 text-xs md:text-sm font-semibold uppercase text-gray-500">
                    ...
                    <th className="py-3.5 px-4 md:px-6 text-center">
                      Audit Status
                    </th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-100 text-sm md:text-base">
                  {PROJECT_DATA_SPECS.boqSummary.map((item, idx) => {
                    ...
                    return (
                      ...
                      >
                        <td className="py-4 px-4 md:px-6 font-semibold text-gray-900">
                          {item.item}
                        </td>
                        <td className="py-4 px-4 md:px-6 font-mono text-right text-gray-900">
                          {item.budget}
                        </td>
                        <td className="py-4 px-4 md:px-6 font-mono text-right text-gray-900">
                          {item.committed}
                        </td>
                        <td className="py-4 px-4 md:px-6 font-mono text-right text-gray-500">
                          {item.balance}
                        </td>
                        <td className="py-4 px-4 md:px-6 font-mono text-right font-bold">
                          <span
                            className={`inline-block px-2.5 py-1 rounded-full text-xs md:text-sm font-medium ${
                              isFavorable
                        ...
                        <td className="py-4 px-4 md:px-6 text-center">
                          <span className="inline-flex items-center gap-1 text-xs md:text-sm font-medium text-green-700 bg-green-50 border border-green-200 px-3 py-1 rounded-full">
                            Verified
                          </span>
                        </td>
                      </tr>
        ...
        {activeTab === 'contractors' && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {PROJECT_DATA_SPECS.contractors.map((c, i) => (
              <div
                key={i}
                className="group bg-white rounded-xl border border-gray-200 p-5 md:p-6 shadow-sm transition-all duration-200 hover:shadow-md hover:-translate-y-0.5 hover:border-gray-300 flex flex-col justify-between"
              >
                <div>
                  <div className="flex items-start justify-between gap-4 mb-2">
                    ...
                    <span className="inline-flex items-center px-2.5 py-1 rounded-full bg-amber-50 text-amber-700 border border-amber-200 text-xs md:text-sm font-medium">
                      ★ {c.rating}
                    </span>
                  </div>
                  ...
                  <div className="bg-gray-50 rounded-lg border border-gray-100 p-4 space-y-2 mb-4">
                    <div className="flex justify-between items-center text-sm">
                      <span className="text-gray-500 font-medium">Deployed Workforce</span>
                      <span className="font-mono font-bold text-gray-900 text-base">{c.workers} Workers</span>
                    </div>
                    <div className="flex justify-between items-center text-sm">
